...
export const resolvers = {
  Query: {
    ...
    searchFavoriteMovies: async (_: any, { userId, query }: { userId: string; query: string }) => {
      const user = await User.findById(userId);
      if (!user) {
        throw new Error('User not found');
      }
      ...
      for (const imdbId of user.favoriteMovies) {
        try {
          ...
          const searchText = `${movie.Title} ${movie.Plot} ${movie.Actors} ${movie.Director} ${movie.Genre}`.toLowerCase();
  ...
  Mutation: {
    ...
    addFavoriteMovie: async (_, { userId, imdbId }: { userId: string; imdbId: string }) => {
      try {
        const user = await User.findById(userId);
        if (!user) {
          throw new Error('User not found');
    ...
    removeFavoriteMovie: async (_, { userId, imdbId }: { userId: string; imdbId: string }) => {
      try {
        const user = await User.findById(userId);
        if (!user) {
          throw new Error('User not found');
